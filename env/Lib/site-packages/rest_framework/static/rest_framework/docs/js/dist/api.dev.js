...
function normalizeHTTPHeader(str) {
  ...
  return (str.charAt(0).toUpperCase() + str.substring(1)).replace(/-(.)/g, function ($1) {
...
$(function () {
  ...
  var $authSessionModal = $('#auth_session_modal'); // Language Control
  ...
  $('form.api-interaction').submit(function (event) {
    ...
    function responseCallback(response, responseText) {
      ...
      $toggleView.removeClass('hide'); // Fill in the "200 OK" display.
      ...
      $meta.removeClass('hide'); // Fill in the Raw HTTP response display.
